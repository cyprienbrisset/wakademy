...
// Script simple pour importer les données MongoDB TopCream
...
async function importFromMongoDB() {
  console.log('🚀 Début de l\'importation MongoDB → Supabase')
  ...
  try {
    ...
    // Récupérer et traiter chaque document (limiter à 5 pour test)
    ...
    while (await cursor.hasNext()) {
      ...
      try {
        // Vérifier si déjà importé en utilisant le titre comme clé unique temporaire
        const { data: existing } = await supabase
          .from('contents')
          .select('id')
          .eq('title', mongoDoc.name)
          .single()
        
        if (existing) {
          console.log(`⏭️ Déjà importé: ${mongoDoc.name}`)
          continue
        }
        
        // Extraire l'URL du fichier depuis MongoDB
        const fileUrl = mongoDoc.contentMedia?.contentMediaURL?.[0]?.url || null
        
        // Préparer les données pour Supabase (colonnes minimales absolues)
        const contentData = {
          ...
          metadata: {
            ...
            thumbnailURL: mongoDoc.contentMedia?.contentMediaURL?.[0]?.thumbnailURL,
